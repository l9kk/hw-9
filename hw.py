...
def missing_elements(my_list: list) -> list:
    ...
    for i in range(1, len(my_list)):
        if my_list[i] > my_list[i - 1] + 1:
            result.extend(range(my_list[i - 1] + 1, my_list[i]))
...
def count_in_range(my_list: list, start: int, end: int) -> int:
    unique = {x for x in my_list if start <= x <= end}
    return len(unique)
...
def least_frequent(my_list: list) -> int:
    ...
    for i in my_list:
        if i in hash_map:
            ...
            hash_map[i] = 1
    mn = min(hash_map.values())
    res = [key for key, value in hash_map.items() if value == mn]
    return res[0]
